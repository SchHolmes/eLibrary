...
import com.javafee.tabbedpane.books.frames.BookAddModFrame;
...
public class BookAddModEvent {
	...
	private void onClickBtnAccept(Context context) {
		if (context == Context.ADDITION) {
			if (validateTablesSelection()) {
				...
				LogGuiException.logWarning("Rows not selected",
	...
	private void modificateBook() {
		try {
			...
			LogGuiException.logError("Incorrect data error. Check number of page, tomes data.", "Incorrect data", e);
		}
	}
